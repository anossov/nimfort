...
proc newWorld*(r: RenderSystem): World =
  result = World(
    R: r,
    cube: newMesh(cubeVertices, newTexture("assets/test.png")),
    cubes: newSeq[Transform](),
  )

  result.R.view = newTransform([0.0'f32, 0.0, 2.0], [0.0'f32, 0.0, 0.0], [1.0'f32, 1.0, 1.0])

  result.cubes.add(newTransform([0.0'f32, 0.0, 0.0]))
  result.cubes.add(newTransform([-1.0'f32, 0.0, 0.0], zero, [0.3'f32, 0.3, 0.3]))
