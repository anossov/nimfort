...
proc initWorld*() =
  ...
  TheWorld.sun.attach(newDirLight(color=vec(0.0, 0.0, 0.0), position=vec(20, 25, 10), shadows=true))
  newEntity("light").attach(newPointLight(color=vec(0, 0, 0.1), position=vec(5.5, 2.0, 5.5), radius=15.0))
  newEntity("light").attach(newPointLight(color=vec(0.1, 0, 0), position=vec(-5.5, 2.0, 5.5), radius=15.0))
  ...
  TheWorld.spot.attach(newSpotLight(vec(-2.4, 12, -1), target=vec(-2.8, 0, -2), color=vec(1, 1, 0.1), angle=20, falloff=40, shadows=true))
